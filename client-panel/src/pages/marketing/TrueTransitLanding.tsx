...
function CampaignCards() {
  const cards = [
    ...
    ['Keep the promise moving', 'TrueTransit helps your team take the next best action, every time.', '\u221e', 'POSSIBILITY', 'tone-coral'],
  ]
...
const pageData = {
  ...
  about: { eyebrow: 'ABOUT TRUETRANSIT', title: 'Built for the people who keep moving.', copy: 'We believe logistics is a creative problem: make the path clearer, and more good things can travel further.', icon: HeartIcon, image: '/assets/shipray-strength-couriers.jpg', imageAlt: 'Courier partner preparing a delivery', cards: [['Clarity first', 'Simple rates, readable milestones and practical next actions.', Sparkles], ['Human when needed', 'Real support for the exceptions software cannot resolve alone.', Headphones], ['Always improving', 'Better decisions informed by route and performance signals.', BarChart3], ['Long-term thinking', 'Build the infrastructure that makes tomorrow feel possible.', ShieldCheck]] },
}
...
function ToolShell({ eyebrow, title, copy, children, image }) { return <section className="tool-page"><div className="shell tool-grid"><div className="tool-intro"><Eyebrow icon={Scale}>{eyebrow}</Eyebrow><h1>{title}</h1><p>{copy}</p><ul className="check-list"><li><Check size={16} /> Clear inputs, useful outputs</li><li><Check size={16} /> Built around Indian routes</li><li><Check size={16} /> No guesswork at checkout</li></ul>{image && <img className="tool-art-image" src={image} alt="TrueTransit shipping tool" loading="lazy" decoding="async" />}</div>{children}</div></section> }
